...
* @prop {String} error_channel The channel ID the guild want to ouput debug info in
*/
class Guild extends Base {
    ...
    update(data) {
		...
		/**
		 * Load settings in to this.guildDoc, or make if it doesn't exist.
		 */
		this.loadSettings();
    }
    ...
    */
	...
	/**
	 * Loads Guild settings. Create and assign if it doesn't exist.
	 * @returns {Promise<Guild>}
	 */
	async loadSettings() {
		let doc = await this.db.settings.findOne({_id:this.id});
		if(!doc) return this.addGuildToDB();
		this.guildDoc = doc.toObject();
		return this;
	}
	
	/**
	 * Checks if the guild is stored in a collection
	 * @arg {Object} model The Mongoose model to apply search to
	 * @returns {Promise<Boolean>}
	 */
	async existInDB(model) {
		return await !!model.find({_id:this.id}, {limit:1});
	}

	/**
	 * Adds guild to settings database if it doesn't exist
	 * @arg {Object} guildSettingsModel The guild settings model
	 * @returns {Promise<Guild>} The stored document
	 */
	async addGuildToDB(guildSettingsModel) {
		return new Promise((resolve,reject) => {
			try {
				if(await existsInDb(model)) return resolve(this);

				// Create it
				let doc = new guildSettingsModel({
					_id: this.id,
					name: this.name,
					...
					enabledChannels: Object.fromEntries(this.channels.map(e=>[e,true])),
					enabledCommands: Object.fromEntries(this._client.commands.map(e=>[e,true]))
				});
				// Save it
				doc.save((err,doc) => {
					if(err) throw err;
					this.guildDoc = doc.hasOwnProperty("toObject")?doc.toObject():doc;
					return resolve(this);
				});
			} catch(err) {
				return reject(err);
	...
	/**
	 * Sends a debug message in the guild dedicated debug channel, if they have one set.
	 * @abstract
	 */
	inform(message) {
		if(this.error_channel) {
			try {
				// Send debug message to the debug channel
				this._client.createMessage(this.error_channel, message);
			} catch(err) {
				// Error sending to debug channel, try owner.
				if(err.code) {
					this._client.createMessage(this._client.getDMChannel(this.ownerID), `**Error:** Could not send debug message in <#${this.error_channel}>: Discord error ${err.code}`);
				} else {
					this._client.createMessage(this._client.getDMChannel(this.ownerID), `**Error:** Could not send debug message in <#${this.error_channel}>: Unknown error.`);
				}
			}
		}
	}
